...
function Sync(options) {
	...
	var syncIndexes = function (name, indexes, cb) {
		Dialect.getCollectionProperties(db, name, function(err, db_properties) {
			...
			Dialect.getCollectionIndexes(db, name, function (err, db_indexes) {
				...
				for (var i = 0; i < indexes.length; i++) {
					isKnownColumn = true;
					for (var j in indexes[i].columns) {
						if (!db_properties.hasOwnProperty(indexes[i].columns[j])) {
							isKnownColumn = false;
						}
					}
					if (!isKnownColumn) {
				...
				for (var i in db_indexes) {
					...
					total_changes += 1;
					isKnownColumn = true;
					for (var j in db_indexes[i].columns) {
						if (!db_properties.hasOwnProperty(db_indexes[i].columns[j])) {
							isKnownColumn = false;
						}
					}
					if (!isKnownColumn) {
